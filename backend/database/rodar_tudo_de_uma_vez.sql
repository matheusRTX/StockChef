...
CREATE TABLE usuarios (
    ...
    -- Preenchido só para Administrador: código único do estabelecimento,
    -- usado pelo Funcionario para se vincular no cadastro.
    codigo_estabelecimento VARCHAR(10) UNIQUE,
    -- Preenchido só para Funcionario: id_usuario do Administrador dono
    -- do estabelecimento ao qual ele está vinculado.
    id_estabelecimento INT,
    -- Administrador nasce 'Aprovado'; Funcionario nasce 'Pendente' e só
    -- opera no sistema após aprovação do Administrador.
    status ENUM('Pendente','Aprovado','Recusado') NOT NULL DEFAULT 'Aprovado',
    PRIMARY KEY (id_usuario),
    CONSTRAINT fk_usuarios_estabelecimento

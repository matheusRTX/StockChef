...
class ProdutoController:
    """Controller responsável pelas rotas de Produto.

    O Blueprint acima é usado apenas como camada de registro das rotas;
    toda a lógica de requisição/resposta fica encapsulada aqui.
    """
    ...
    @staticmethod
    def listar():
        produtos = ListarProdutosService.execute(id_usuario=session.get('id_estabelecimento'))
        return jsonify([ProdutoController._produto_para_json(p) for p in produtos])
    ...
    def criar():
        ...
        try:
            ...
            return jsonify(ProdutoController._produto_para_json(produto)), 201
    ...
    def buscar(id_produto):
        try:
            ...
            return jsonify(ProdutoController._produto_para_json(produto))
        except Exception as e:
...
produto_bp.before_request(ProdutoController.exigir_login)

produto_bp.add_url_rule('', view_func=ProdutoController.listar, methods=['GET'])
produto_bp.add_url_rule('', view_func=ProdutoController.criar, methods=['POST'])
produto_bp.add_url_rule('/<int:id_produto>', view_func=ProdutoController.buscar, methods=['GET'])
produto_bp.add_url_rule('/<int:id_produto>', view_func=ProdutoController.atualizar, methods=['PUT'])
produto_bp.add_url_rule('/<int:id_produto>', view_func=ProdutoController.remover, methods=['DELETE'])

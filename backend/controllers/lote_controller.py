...
def listar():
    ...
    return jsonify([_lote_para_json(l) for l in lotes])
...
def criar():
    ...
    try:
        ...
        return jsonify(_lote_para_json(lote)), 201
    except Exception as e:
        return jsonify({"erro": str(e)}), 400


@lote_bp.route('/<int:id_lote>', methods=['GET'])
def buscar(id_lote):
    try:
        lote = BuscarLoteService.execute(id_lote)
        return jsonify(_lote_para_json(lote))
    except Exception as e:
        return jsonify({"erro": str(e)}), 404


@lote_bp.route('/<int:id_lote>', methods=['PUT'])
...
@lote_bp.route('/<int:id_lote>', methods=['DELETE'])
def remover(id_lote):
    try:

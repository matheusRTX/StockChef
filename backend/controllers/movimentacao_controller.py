...
class MovimentacaoController:
    """Controller responsável pelas rotas de Movimentação."""
    ...
    @staticmethod
    def listar():
        movimentacoes = ListarMovimentacoesService.execute(id_usuario=session.get('id_estabelecimento'))
        return jsonify([MovimentacaoController._movimentacao_para_json(m) for m in movimentacoes])
    ...
    def criar():
        ...
        try:
            ...
            return jsonify(MovimentacaoController._movimentacao_para_json(movimentacao)), 201
    ...
    def buscar(id_movimentacao):
        try:
            ...
            resposta = MovimentacaoController._movimentacao_para_json(movimentacao)
            resposta['itens'] = [MovimentacaoController._item_para_json(i) for i in itens]
    ...
    def atualizar(id_movimentacao):
        ...
        try:
            ...
            return jsonify(MovimentacaoController._movimentacao_para_json(movimentacao))
        except Exception as e:
            return jsonify({"erro": str(e)}), 400
...
movimentacao_bp.before_request(MovimentacaoController.exigir_login)

movimentacao_bp.add_url_rule('', view_func=MovimentacaoController.listar, methods=['GET'])
movimentacao_bp.add_url_rule('', view_func=MovimentacaoController.criar, methods=['POST'])
movimentacao_bp.add_url_rule('/<int:id_movimentacao>', view_func=MovimentacaoController.buscar, methods=['GET'])
movimentacao_bp.add_url_rule('/<int:id_movimentacao>', view_func=MovimentacaoController.atualizar, methods=['PUT'])
movimentacao_bp.add_url_rule('/<int:id_movimentacao>', view_func=MovimentacaoController.remover, methods=['DELETE'])
